...
* [💡 Motivation](#motivation)
  * [👩‍💼 서비스 이용자(사장님)의 관점](#-서비스-이용자사장님의-관점)
  * [🧑‍💻 방문자(홈페이지 이용자)의 관점](#-방문자홈페이지-이용자의-관점)
...
* [🖥️ Development](#️-development)
  ...
  * [3. 챗봇과의 대화 및 DB 메시지를 어떻게 불러올까?](#3-챗봇과의-대화-및-db-메시지를-어떻게-불러올까)
    * [3-1. 세션 단위 메시지 조회 구조](#3-1-세션-단위-메시지-조회-구조)
  * [4. 크롤러 기능을 어떻게 구현했을까?](#4-크롤러-기능을-어떻게-구현했을까)
  * [5. 이메일 인증 기반 회원가입은 어떻게 설계, 구현했는가?](#5-이메일-인증-기반-회원가입은-어떻게-설계-구현했는가)
    * [5-1. 이메일 인증이 필요한 이유](#5-1-이메일-인증이-필요한-이유)
    * [5-2. VerificationToken 기반 인증 구조](#5-2-verificationtoken-기반-인증-구조)
    * [5-3. Resend를 활용한 인증 메일 발송](#5-3-resend를-활용한-인증-메일-발송)
...
### 3. 홈페이지 방문자와 챗봇의 대화를 사장님이 알 수 있을까?
---

Desker 대시보드에서는 사장님이 방문자와 챗봇 간의 대화 흐름을 쉽게 확인할 수 있도록 챗봇 ID → 세션 목록 → 메시지 내역 구조의 조회 시스템을 구현했습니다.
사장님은 자신의 챗봇에 연결된 모든 세션 목록을 확인할 수 있으며 특정 세션을 선택하면 해당 방문자와 챗봇 사이의 대화 기록이 즉시 로드되도록 구현했습니다.

대화목록 페이지 진입 시 sessionId를 기반으로 해당 방문자의 메시지 히스토리를 API에서 불러와 채팅 UI에 주입하는 구조로 설계했습니다.
...
useEffect(() => {
  const loadMessages = async () => {
    ...
    if (data.messages?.length) {
      setMessages(data.messages); // 세션별 메시지 상태 초기화
    }
  };
...
왜 이런 구조가 필요했는가?
Desker 서비스는 여러 방문자가 동시에 챗봇과 대화할 수 있기 때문에
사장님이 특정 방문자의 메시지를 찾기 어려운 문제가 있었습니다.

또한 초기 버전에서는 메시지가 단일 리스트로만 제공되어
“누가 언제 어떤 대화를 했는지” 맥락을 파악하기 어려웠습니다.

이 문제를 해결하기 위해 방문자 단위로 세션을 분리하고,
각 세션을 통해 개별 메시지 흐름을 확인할 수 있는 구조가 필요했습니다.

이렇게 함으로써 사장님은 방문자의 문의 히스토리를 빠르게 파악하고, 반복되는 질문이나 주요 관심사를 확인하며 서비스 운영 품질을 개선할 수 있습니다.

<br>

### 4. 챗봇 학습을 위한 크롤러는 왜 구현했는가?
---
...
### 5. 이메일 인증 기반 회원가입은 어떻게 설계, 구현했는가?
---

Desker 서비스에서는 보안성과 서비스 신뢰도를 높이기 위해  
회원가입 → 이메일 인증 → 로그인 허용 구조의 인증 플로우를 설계, 구현했습니다.

단순히 이메일과 비밀번호만으로 계정을 활성화할 경우  
타인의 이메일 도용, 잘못된 이메일 입력, 스팸 계정 생성 등의 문제가 발생할 수 있다고 판단했습니다.  
특히 Desker는 사장님이 실제로 사용하는 서비스이기 때문에 계정의 신뢰성이 매우 중요했습니다.

이에 따라 회원가입 시점에서는 계정을 즉시 로그인 상태로 만들지 않고
이메일 인증을 완료한 사용자만 로그인이 가능하도록 구조를 설계했습니다.

#### 이메일 인증 흐름 요약
1. 사용자가 회원가입 폼 제출
2. Zod 기반 유효성 검증
3. User 생성 (user의 emailVerified = null)
4. 이메일 인증용 토큰 생성
5. VerificationToken 테이블에 토큰 저장
6. 인증 링크를 이메일(Resend)로 발송
7. 사용자가 이메일 내 인증 링크 클릭
8. 토큰 검증 후 user의 emailVerified 업데이트,  VerificationToken 테이블에 토큰 삭제
9. 로그인 페이지로 리다이렉션
10. 로그인 시 emailVerified 여부를 확인하여 인증된 계정만 로그인 허용

위와 같은 흐름을 구현하기 위해서는 이메일 발송 서비스인 Resend를 사용하였습니다.
React 기반 이메일 템플릿 지원과 Next.js App Router와의 호환성 그리고 개발 단계에서 빠른 테스트 가능가 가능하여 Resend를 채택하였습니다. 

```tsx
// Resend를 이용하여 이메일 인증 송부 기능 구현 중 일부분
await resend.emails.send({
  from: "Desker AI <onboarding@your-domain.com>",
  to: [email],
...
| Resend를 사용한 이메일 인증 |
|:--:|
| <img width="1252" height="687" alt="Image" src="https://github.com/user-attachments/assets/fe2608df-207f-4b47-923d-42164d47a767" /> |

Resend의 React 컴포넌트 기반 이메일 템플릿을 사용하였습니다.
위 이메일 템플릿은 빠른 기능 구현을 위하여 기본적인 UI만 도입하였습니다. 추후 서비스 톤앤매너에 맞는 인증 메일 UI를 구성할 예정입니다.

<br>

#### 토큰 기반 이메일 인증 방식 선택 이유

이메일 인증 구현 시 두 가지 방식이 후보였습니다.

1. JWT(jose)를 이용한 서명 토큰 방식  
2. DB에 토큰을 저장하는 방식

Desker에서는 DB 기반 토큰 관리 방식을 선택했습니다.
그 이유는 다음과 같습니다.
...
이를 위해 Prisma의 `VerificationToken` 테이블 구조를 그대로 활용했습니다.
...
### UI & UX 개선
* 	방문자 연락처, 개인설정 페이지 구현을 마무리 해야 합니다. 	
